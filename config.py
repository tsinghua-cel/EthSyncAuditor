...
SUBSYSTEM_DOMAINS: list[Domain] = [
    Domain(
        ...
        retrieval_queries=[
            ...
            "inbound receive-side UDP request rate limit vs outbound self limiter",
            "discv5 service socket recv_from read loop per-IP inbound quota",
        ],
        aspect_hints=[
            ...
            {"id": "receive_side_udp_rate_limiting",
             "desc": "Whether the discv5 receive path (the socket recv/read loop that ingests inbound UDP datagrams before decoding) applies its own rate limit distinct from the outbound/self rate limiter — e.g. a per-IP or per-socket inbound quota checked before a packet is parsed. State whether inbound and outbound limits are unified or separate, and what happens when the inbound limit is exceeded (drop silently, ban, log)."},
        ],
    ),
    Domain(
        id="peer_scoring",
    ...
    Domain(
        ...
        entry_keywords=[
            ...
            "worstpeers", "excesspeers", "trimpeers",
        ],
        entry_path_markers=[
        ...
        retrieval_queries=[
            ...
            "prune worst-scoring peers when over target count selection criteria",
            "peer pruning exempt trusted inbound outbound long-lived subnet peers",
        ],
        aspect_hints=[
            {"id": "peer_pruning_policy",
             "desc": "When and how excess peers are pruned once the peer count exceeds a target/max: what selection criteria decide which peers to drop first (e.g. lowest score, most recently connected, non-subnet peers), how many are pruned per pass, and whether trusted/inbound/long-lived-subnet peers are exempt from pruning."},
        ],
    ),
    Domain(
        id="validator_duty_management",
        name="Validator Duty Management",
        kind="subsystem",
        description=(
            "Attester/proposer duty caching, refresh windows, epoch-gap "
            "tolerance, and reorg-driven duty invalidation"
        ),
        extract_fsm=False, extract_parameters=True, max_call_depth=7,
        entry_keywords=[
            "dutie", "duty", "attesterduties", "proposerduties", "dutycache",
            "dutymanager", "committeeassignment", "lookahead", "dutiesservice",
            "refetchduties", "invalidateduties", "epochtransition",
        ],
        entry_path_markers=[
            "duties", "validator/client", "services/attestation",
            "services/block", "duty_manager", "epoch_processing",
        ],
        retrieval_queries=[
            "attester duty query lookahead how far ahead duties are fetched cached",
            "proposer duty epoch gap tolerance stale duty recompute",
            "validator duty cache invalidate on reorg refetch duties",
            "duty assignment epoch boundary refresh interval next epoch",
            "stale duty detection dependent root change requery committee",
            "duties service current epoch next epoch subscription lookahead",
        ],
        aspect_hints=[
            {"id": "attestation_duty_query_window",
             "desc": "How far ahead of the current epoch attester duties are queried/cached (e.g. current epoch only, current+next, N-epoch lookahead), and on what cadence (once per epoch, per slot, on demand) duties are re-queried from the beacon node."},
            {"id": "proposal_epoch_gap_tolerance",
             "desc": "Whether the client tolerates a gap between the epoch a proposer duty was computed for and the current epoch before treating it as stale, how large that tolerance is (in epochs/slots), and what happens when the gap is exceeded (recompute vs. use the stale duty as-is)."},
            {"id": "reorg_driven_stale_duty_window",
             "desc": "Whether a chain reorg (dependent-root change) triggers a refetch/invalidation of cached attester or proposer duties, how large a reorg (in slots/epochs, or 'any dependent-root change') is needed to trigger the refresh, and what the client does with in-flight duties computed against the now-stale root."},
            {"id": "duty_cache_invalidation_trigger",
             "desc": "The concrete trigger(s) that invalidate the duty cache besides reorgs (e.g. epoch transition, finality change, validator (de)activation), and whether invalidation is partial (only affected validators) or full (entire cache)."},
        ],
    ),
]
