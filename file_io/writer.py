...
def _format_evidence_line(client: str, ev: Any) -> str:
    """Render one ``- {client}: ...`` evidence bullet, tolerating malformed data.

    ``ev`` is normally a ``{file, function, lines}`` dict, but upstream LLM
    output can occasionally produce a bare string or other shape. Render
    whatever we get instead of raising.
    """
            ...
            for diff in vuln_diffs:
                ...
                if isinstance(evidence, dict) and evidence:
                    ...
                    for client, ev in evidence.items():
                        if ev:
                            lines.append(_format_evidence_line(client, ev))
                lines.append("")
                v_idx += 1
