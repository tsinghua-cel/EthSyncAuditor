...
class PreprocessingError(RuntimeError):
    """Raised when preprocessing cannot produce usable artifacts for a client.

    This is a hard-fail, deliberately not swallowed: a client with zero
    extracted symbols means CODE_BASE_PATH/<client_name> has no discoverable
    source (wrong path, repo not cloned, etc.). Continuing silently in that
    case used to let the whole multi-hour, multi-LLM-call pipeline run to
    completion with empty BM25/vector indices, so Phase 3 verification could
    never resolve any evidence and every diff ended up INSUFFICIENT_EVIDENCE.
    """


def _symbols_count(client_name: str) -> int:
    """Return the number of symbols on disk for *client_name* (0 if missing)."""
    path = PREPROCESS_PATH / f"{client_name}_symbols.json"
    if not path.exists():
        return 0
    try:
        with open(path) as f:
...
def _artifacts_exist(client_name: str) -> bool:
    """Check if all preprocessing artifacts already exist *and are non-trivial*
    for *client_name*. A ``0``-symbol / empty index state is treated as
    "does not exist" so a previously-broken source path gets retried instead
    of being cached as permanently "ready"."""
    base = PREPROCESS_PATH
    if _symbols_count(client_name) <= 0:
        return False
    bm25_path = base / f"{client_name}_bm25.pkl"
    chroma_dir = base / f"{client_name}_chroma"
    return all([
        (base / f"{client_name}_callgraph.json").exists(),
        bm25_path.exists() and bm25_path.stat().st_size > 0,
...
def run_preprocessing(client_name: str, force_rebuild: bool = False) -> dict[str, bool]:
    ...
    ------
    PreprocessingError
        If no symbols could be extracted from ``CODE_BASE_PATH/client_name``.
        This is a fatal configuration/source-fetch problem, not something the
        rest of the pipeline can meaningfully work around.
    """
    ...
    if not symbols:
        raise PreprocessingError(
            f"[run_preprocessing] client={client_name!r} — 0 symbols extracted "
            f"from {CODE_BASE_PATH / client_name}. Expected the client source "
            f"tree to live at CODE_BASE_PATH/{client_name} (see getcode.sh, "
            f"which clones into ./code/<client>). Check that the repo was "
            f"actually fetched there before re-running."
        )
    ...
    chroma_dir = PREPROCESS_PATH / f"{client_name}_chroma"
    status["vector_index_ready"] = chroma_dir.exists() and any(chroma_dir.iterdir())

    # Task D
    _build_bm25_index(client_name, symbols)
    bm25_path = PREPROCESS_PATH / f"{client_name}_bm25.pkl"
    status["bm25_index_ready"] = bm25_path.exists() and bm25_path.stat().st_size > 0
...
def run_all_preprocessing(force_rebuild: bool = False) -> dict[str, dict[str, bool]]:
    ...
    ------
    PreprocessingError
        If any client ends up without a working BM25 or vector index — those
        are exactly the artifacts Phase 3 evidence retrieval depends on, so a
        client left in that state would silently poison every later
        verification verdict for it.
    """
    ...
    broken = [
        client for client, status in results.items()
        if not (status.get("bm25_index_ready") and status.get("vector_index_ready"))
    ]
    if broken:
        raise PreprocessingError(
            f"[run_all_preprocessing] BM25/vector index build failed for: "
            f"{broken}. Evidence retrieval (Phase 3) cannot work without "
            f"these — aborting before any LLM calls are made. Statuses: "
            f"{ {c: results[c] for c in broken} }"
        )
    return results
